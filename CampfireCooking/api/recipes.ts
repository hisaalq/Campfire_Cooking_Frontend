...
const getAllRecipes = async () => {
    try{
    console.log("Fetching recipes from:", instance.defaults.baseURL + "/api/recipes");
    const {data} = await instance.get<{count: number, data: RecipeInfo[], success: boolean}>("/api/recipes");
...
const getRecipeById = async (id: string) => {
    try{
    const {data} = await instance.get<{ success: boolean; data: RecipeInfo }>(`/api/recipes/${id}`);
  ...
  const getRecipesByCategory = async (categoryId: string) => {
    try{
    const {data} = await instance.get<RecipeInfo[]>(`/api/recipes/category/${categoryId}`);
    return data;
    } catch (error) {
